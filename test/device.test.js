...
import { DeviceEndpoint } from '../endpoints/device.js'
...
describe('Device Status Endpoint', () => {
    let endpoint
    beforeEach(() => {
        endpoint = new DeviceEndpoint({ host: '127.0.0.1', verbosity: 0 })
    })
    it('can check device info', async() => {
        ...
        const info = await endpoint.getDeviceInfo()
    ...
    it('can check power status', async() => {
        ...
        assert.equal(await endpoint.isOn(), true)
    })
})
